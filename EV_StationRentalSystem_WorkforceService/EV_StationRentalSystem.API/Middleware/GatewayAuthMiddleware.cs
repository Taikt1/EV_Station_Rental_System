...
namespace EV_StationRentalSystem.API.Middleware
{
    public class GatewayAuthMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            _logger.LogInformation("Gateway headers - UserId: {UserId}, Role: {Role}", userId, userRole);
            ...
            if (!string.IsNullOrEmpty(userId))
            {
                ...
                {
                    new Claim("nameid", userId), // Use the same claim type as expected
                    new Claim(ClaimTypes.NameIdentifier, userId)
                };

                _logger.LogInformation("Gateway Auth: Created user identity for {UserId}", userId);
